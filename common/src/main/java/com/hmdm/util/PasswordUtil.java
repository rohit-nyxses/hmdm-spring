...
//        log.debug("🔍 enteredPass to getHashFromMd5: {}", enteredPass);
    ...
    public boolean checkPassword(String password, int length, int strength) {
        ...
        if (strength == PASS_STRENGTH_NONE) return true;
        ...
        for (int n = 0; n < password.length(); n++) {
            ...
            if (i == -1) hasSpecial = true;
            else if (i >= DIGIT_START && i <= DIGIT_END) hasDigits = true;
            else if (i >= ALPHA_LOWER_START && i <= ALPHA_LOWER_END) hasLower = true;
            else if (i >= ALPHA_CAPS_START && i <= ALPHA_CAPS_END) hasCaps = true;
            else if (i >= ALPHA_CHAR_START && i <= ALPHA_CHAR_END) hasSpecial = true;
        }

        if (strength == PASS_STRENGTH_ALPHADIGIT) return hasDigits && hasLower && hasCaps;
        if (strength == PASS_STRENGTH_SPECIAL) return hasDigits && hasLower && hasCaps && hasSpecial;
    ...
    public String generatePassword(int length, int strength) {
        ...
        switch (strength) {
            case PASS_STRENGTH_ALPHADIGIT -> charIntervalEnd = ALPHA_CAPS_END;
            case PASS_STRENGTH_SPECIAL -> charIntervalEnd = ALPHA_CHAR_END;
        }
